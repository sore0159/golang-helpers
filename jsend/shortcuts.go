...
func U2(w http.ResponseWriter) {
	Fail(w, 404, map[string]string{"message": "resource not found"})
}

// YOU SHALL NOT PASS!
func Gandalf(w http.ResponseWriter) {
	Fail(w, 401, map[string]string{"message": "access denied"})
}

// Your request was bad and you should feel bad.
func Zoidberg(w http.ResponseWriter) {
	Fail(w, 400, map[string]string{"message": "bad specification for requested resource"})
}
